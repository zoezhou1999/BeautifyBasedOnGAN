...
parser.add_argument('--use_beauty_score_loss', default=0, help='Use LPIPS perceptual loss; 0 to disable, > 0 to scale.', type=float)
#
# Generator params
...
for batch_index, images_batch in enumerate(tqdm(split_to_batches(ref_images, args.batch_size), total=len(ref_images)//args.batch_size)):
    ...
    for image_path in images_batch:
        cl=beautyrater_model.predict(image_path)
